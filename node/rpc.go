...
func (r *RPCClient) TxPoolSubscribe(ctx context.Context, nm chan *carthage.NodeMessage) (*FullPendingTransactionsSubscription, error) {
	...
	go func() {
		for {
			...
			case tx := <-ch:
				...
				nm <- &carthage.NodeMessage{Data: []byte(m)}
			case err := <-sub.Err():
				fmt.Println(err)
				return
			}
